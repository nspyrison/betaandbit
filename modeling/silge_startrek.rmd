...
subtitle: "Following Julia Silge's analysis"
...
if(F){
  ...
  install.packages(c("textrecipes", "themis", "stopwords", "discrim"))
  install.packages(c("naivebayes", "LiblineaR"))
}
...
is.na(computer_raw) %>% colSums()

if(F) ## Don't print a visual of this.
  naniar::vis_miss(computer_raw)

## Count table; count of unique words by char_type
computer_raw %>%
...
## Finding the top words by char type:
computer_counts <-
  computer_raw %>%
  distinct(value_id, .keep_all = TRUE) %>%
...
First we need to partition our data into splits. Note that we don't have much data to work with, but we are going to use a typical 80% split. We'll also bootstrap in 25 folds to help with this.
...
## Bootstrap 25 folds of the data to help with the small size.
set.seed(234)
...
# Map recipes to models and folds
...
```{r}
set.seed(123)
cores_less1 <- parallel::detectCores() - 1
doParallel::registerDoParallel(cores = cores_less1)

tictoc::tic("map fit_resamples")
computer_rs <-
  comp_models %>%
  workflow_map(
    "fit_resamples",
...
tictoc::toc() ## Running locally: 65.6 sec elapsed
```
...
```{r}
## Packages used
pkgs <- c("tidyverse", "tidymodels", "tidytext", "tidylo",
          "textrecipes", "themis", "stopwords", "discrim",
          "naivebayes", "LiblineaR")
...
- [Interactive list of parsnip models](https://www.tidymodels.org/find/parsnip/)